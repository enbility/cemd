...
func TestMGCPSuite(t *testing.T) {
	suite.Run(t, new(UCMGCPSuite))
}

type UCMGCPSuite struct {
	suite.Suite
...
func (s *UCMGCPSuite) SpineEvent(ski string, entity spineapi.EntityRemoteInterface, event api.UseCaseEventType) {
}
...
func (s *UCMGCPSuite) BeforeTest(suiteName, testName string) {
	...
	s.sut = NewUCMGCP(s.service, s.service.LocalService(), s)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
}
		...
		EntityInformation: []model.NodeManagementDetailedDiscoveryEntityInformationType{
			{
				Description: &model.NetworkManagementEntityDescriptionDataType{
					...
					EntityType: eebusutil.Ptr(model.EntityTypeTypeGridConnectionPointOfPremises),
				},
			},
		},
