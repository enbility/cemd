...
func TestMOCSuite(t *testing.T) {
	suite.Run(t, new(UCMPCSuite))
}

type UCMPCSuite struct {
	suite.Suite
...
func (s *UCMPCSuite) SpineEvent(ski string, entity spineapi.EntityRemoteInterface, event api.UseCaseEventType) {
}
...
func (s *UCMPCSuite) BeforeTest(suiteName, testName string) {
	...
	s.sut = NewUCMCP(s.service, s.service.LocalService(), s)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
