...
func TestLPPServerSuite(t *testing.T) {
	suite.Run(t, new(UCLPPServerSuite))
}

type UCLPPServerSuite struct {
	suite.Suite
...
func (s *UCLPPServerSuite) Event(ski string, device spineapi.DeviceRemoteInterface, entity spineapi.EntityRemoteInterface, event api.EventType) {
}

func (s *UCLPPServerSuite) BeforeTest(suiteName, testName string) {
	...
	s.sut = NewUCLPP(s.service, s.Event)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
		...
		EntityInformation: []model.NodeManagementDetailedDiscoveryEntityInformationType{
			{
				Description: &model.NetworkManagementEntityDescriptionDataType{
					...
					EntityType: eebusutil.Ptr(model.EntityTypeTypeGridGuard),
				},
			},
		},
