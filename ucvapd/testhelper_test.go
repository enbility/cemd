...
func TestVAPDSuite(t *testing.T) {
	suite.Run(t, new(UCVAPDSuite))
}

type UCVAPDSuite struct {
	suite.Suite
...
func (s *UCVAPDSuite) SpineEvent(ski string, entity spineapi.EntityRemoteInterface, event api.UseCaseEventType) {
}

func (s *UCVAPDSuite) BeforeTest(suiteName, testName string) {
	...
	s.sut = NewUCVAPDV(s.service, s.service.LocalService(), s)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
}
...
func setupDevices(
	...
	f := spine.NewFeatureLocal(1, localEntity, model.FeatureTypeTypeMeasurement, model.RoleTypeClient)
	localEntity.AddFeature(f)
	f = spine.NewFeatureLocal(2, localEntity, model.FeatureTypeTypeElectricalConnection, model.RoleTypeClient)
	localEntity.AddFeature(f)
	f = spine.NewFeatureLocal(3, localEntity, model.FeatureTypeTypeDeviceConfiguration, model.RoleTypeClient)
	localEntity.AddFeature(f)
		...
		EntityInformation: []model.NodeManagementDetailedDiscoveryEntityInformationType{
			{
				Description: &model.NetworkManagementEntityDescriptionDataType{
					...
					EntityType: eebusutil.Ptr(model.EntityTypeTypePVSystem),
				},
			},
		},
