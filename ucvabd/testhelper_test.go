...
func (s *UCVABDSuite) SpineEvent(ski string, entity spineapi.EntityRemoteInterface, event api.UseCaseEventType) {
}
...
func (s *UCVABDSuite) BeforeTest(suiteName, testName string) {
	...
	s.sut = NewUCVABD(s.service, s.service.LocalService(), s)
	s.sut.AddFeatures()
	s.sut.AddUseCase()
